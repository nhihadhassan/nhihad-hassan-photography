...
function reasonClass(reason: string | null, success: boolean | null) {
  ...
  if (reason === "locked" || reason === "not_enabled") return "bg-[#b98257]/20 text-[#9b744f]";
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="grid grid-cols-3 gap-3 text-sm">
          ...
          <div className="rounded-md border border-[#17130f]/10 bg-[#fbf8f1] p-3">
            ...
            <span className="text-xs text-[#17130f]/55">Started</span>
          </div>
          <div className="rounded-md border border-[#17130f]/10 bg-[#fbf8f1] p-3">
            <span className="block text-xl font-semibold text-[#8a2f24]">{stats.failure}</span>
            <span className="text-xs text-[#17130f]/55">Rejected</span>
          </div>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-[#17130f]/10">
              {logs.map((log) => (
                <tr key={log.id}>
                  ...
                  <td className="px-4 py-3">
                    <span className="inline-flex items-center gap-1.5 rounded-full bg-[#17130f]/8 px-2.5 py-1 text-xs font-medium text-[#17130f]/68">
                      {log.scope === "selects" ? (
                  ...
                  <td className="px-4 py-3 text-[#17130f]/72">{log.photo_count}</td>
                  <td className="px-4 py-3">
                    <span
                      className={
