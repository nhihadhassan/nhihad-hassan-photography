...
export default async function AgreementSigningPage({
  ...
  const photographerName = snapshot?.photographerName ?? brandConfig.name;
  ...
  // Every required client signature is in, but the agreement is only final once
  // the photographer countersigns it.
  const clientSigningComplete =
    Boolean(request.client_submitted_at) || (requiredSigners.length > 0 && remainingSigners.length === 0);
  const fullySigned = Boolean(request.signed_at);
  const awaitingCountersignature = clientSigningComplete && !fullySigned;
  const countersignature = signatures.find((signature) => signature.photographer_signed_at);
  const signatureSlot = (
