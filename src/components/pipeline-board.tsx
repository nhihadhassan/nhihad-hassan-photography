...
function ChecklistRow({ label, done }: { label: string; done: boolean }) {
  return (
    <span className="inline-flex items-center gap-1 text-xs text-admin-ink/65">
      {done ? (
  ...
  return (
    <div className="rounded-md border border-admin-ink/10 bg-white/70 p-3">
      <Link href={`/admin/bookings/${card.id}`} className="block">
        ...
        <p className="mt-0.5 text-xs text-admin-ink/65">{card.subtitle}</p>
      </Link>
      ...
      <div className="mt-3 flex items-center justify-between">
        <button
          ...
          className="inline-flex size-7 items-center justify-center rounded-md border border-admin-ink/12 text-admin-ink/65 transition hover:bg-admin-ink/6 disabled:opacity-30"
        ...
        return (
          <div key={stage} className="rounded-lg border border-admin-ink/10 bg-admin-surface p-3">
            <div className="flex items-center justify-between px-1">
              ...
              <span className="text-xs text-admin-ink/65">{inStage.length}</span>
            </div>
