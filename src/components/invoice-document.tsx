...
const STATUS_LABEL: Record<InvoiceStatusKey, string> = {
  draft: "Draft",
  sent: "Sent",
  viewed: "Viewed",
...
function OverallStatusPill({ status }: { status: InvoiceStatusKey }) {
  const tone =
    status === "paid"
      ? "bg-[#e7efe8] text-[#3f6e4a]"
  ...
  return (
    <span className={`inline-flex rounded-full px-3 py-1 text-[0.65rem] font-semibold uppercase tracking-[0.12em] ${tone}`}>
      {STATUS_LABEL[status]}
    </span>
  );
}
    ...
    <dl className="ml-auto mt-8 w-full max-w-sm text-sm tabular-nums">
      ...
      {tax > 0 ? (
        <div className="flex justify-between gap-6 py-2 text-ink/65">
          ...
          <dd className="text-ink">{formatMoney(tax)}</dd>
        </div>
      ) : null}
      <div className="flex justify-between gap-6 py-2 text-ink/65">
        <dt>Total</dt>
...
/**
 * The rendered invoice: hero header, items, totals, and payment details.
 * Shared by the client-facing /invoice/[token] page and the admin's
 * read-only preview at /admin/invoices/[id]/preview, so neither can drift
 * from what the other shows -- the same pattern AgreementDocument uses for
 * contracts.
 */
export function InvoiceDocument({
  ...
  return (
    <div className={variant === "client" ? "min-h-[100dvh] bg-[#f1efec] text-ink print:bg-white" : "bg-[#f1efec] text-ink"}>
      <header className="relative h-[34rem] overflow-hidden bg-charcoal text-soft-white print:hidden sm:h-[35rem]">
        ...
        <div className="absolute inset-x-0 top-0 z-10 px-5 py-6 sm:px-8">
          <div className="mx-auto flex max-w-6xl items-center justify-between">
            <Link
              href={variant === "client" ? "/" : "/admin/invoices"}
            ...
            >
              ...
              {variant === "client" ? "Back to site" : "Back to invoices"}
            </Link>
      ...
      <main className="relative z-20 mx-auto -mt-24 max-w-[52rem] px-4 pb-20 sm:px-6 print:mt-0 print:max-w-none print:px-0 print:pb-0">
          ...
          <section className="px-6 py-8 sm:px-12 sm:py-11 print:px-0 print:py-0">
            <div className="flex flex-wrap items-start justify-between gap-5">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <OverallStatusPill status={invoice.status} />
              </div>
              <div className="flex items-center gap-2 print:hidden">
                <PrintButton iconOnly className="rounded-md border-ink/15 hover:bg-ink/[0.04]" />
                <a
            ...
            {!invoice.paidInFull && invoice.balance > 0 && invoice.status !== "cancelled" ? (
              <a
        ...
        <section id="payment-details" className="mt-3 bg-[#fffefa] px-6 py-9 shadow-[0_12px_40px_rgba(30,25,20,0.06)] sm:px-12 sm:py-11 print:mt-8 print:shadow-none">
          ...
          <div className="mt-8 max-w-2xl border-t border-ink/10 pt-7">
            ...
            {invoice.status === "cancelled" ? (
              <p className="mt-3 text-sm leading-6 text-ink/70">This invoice was cancelled. No payment is required.</p>
            ) : invoice.paidInFull ? (
              <p className="mt-3 text-sm leading-6 text-ink/70">Payment received in full. Thank you.</p>
            ) : (
              <p className="mt-3 whitespace-pre-line text-sm leading-6 text-ink/70">{invoice.paymentInstructions}</p>
            )}
