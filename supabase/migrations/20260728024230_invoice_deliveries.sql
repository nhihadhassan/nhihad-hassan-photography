-- Auditable invoice email delivery.
--
...
-- clicked, delayed, and failed messages instead of treating every invoice with
-- a total as already sent.
...
create index invoice_deliveries_booking_created_idx
  on public.invoice_deliveries(booking_id, created_at desc);

create table public.invoice_delivery_events (
  id text primary key,
  delivery_id uuid not null references public.invoice_deliveries(id) on delete cascade,
...
create index invoice_delivery_events_delivery_occurred_idx
  on public.invoice_delivery_events(delivery_id, occurred_at desc);
...
create policy "Deny all non-service-role access"
  on public.invoice_deliveries for all using (false) with check (false);

create policy "Deny all non-service-role access"
  on public.invoice_delivery_events for all using (false) with check (false);

revoke all on table public.invoice_deliveries from anon, authenticated;
